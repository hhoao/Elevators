...
public class Config {
    ...
    private static final ForgeConfigSpec.ConfigValue<List<? extends String>>
            ELEVATOR_BLOCKS_WITH_HEIGHT =
                    BUILDER.defineList(
                            "elevatorBlocksWithHeight", getDefaultElevatorsBlock(), (o) -> true);
        ...
        if (blockMaxHeightMap == null) {
            ...
            for (String blockHeight : ELEVATOR_BLOCKS_WITH_HEIGHT.get()) {
                ...
                if (split.length > 2) {
                    ...
                    blockMaxHeightMap.put(
                            ForgeRegistries.BLOCKS.getValue(new ResourceLocation(substring)),
                            Integer.valueOf(split[2]));
                } else {
                    blockMaxHeightMap.put(
                            ForgeRegistries.BLOCKS.getValue(new ResourceLocation(blockHeight)), -1);
                }
            }
        }
    ...
    public static int setAllowElevatingThroughBlocks(CommandContext<CommandSource> context) {
        ...
        context.getSource()
                .sendFeedback(
                        new StringTextComponent("Set allowElevatingThroughBlocks to " + value),
                        true);
    ...
    public static int setMaxTeleportHeight(CommandContext<CommandSource> context) {
        ...
        context.getSource()
                .sendFeedback(new StringTextComponent("Set maxTeleportHeight to " + value), true);
        return Command.SINGLE_SUCCESS;
    }
        ...
        context.getSource()
                .sendFeedback(
                        new StringTextComponent(
                                String.format(
                                        "Add elevatorBlock: %s, height: %s",
                                        block.getTranslationKey(),
                                        height == -1 ? getDefaultMaxTeleportHeight() : height)),
                        true);
        return Command.SINGLE_SUCCESS;
    }
    ...
    public static int removeElevatorBlock(CommandContext<CommandSource> context) {
        ...
        context.getSource()
                .sendFeedback(
                        new StringTextComponent(
                                "Remove elevatorBlock: " + block.getTranslationKey()),
                        true);
        return Command.SINGLE_SUCCESS;
    }

    private static void refreshConfig(Map<Block, Integer> elevatorBlockMaxHeightMap) {
        ELEVATOR_BLOCKS_WITH_HEIGHT.set(
                elevatorBlockMaxHeightMap.entrySet().stream()
                        .map(
                                blockHeight ->
                                        ForgeRegistries.BLOCKS
                                                        .getKey(blockHeight.getKey())
                                                        .toString()
                                                + (blockHeight.getValue() == -1
                                                        ? ""
