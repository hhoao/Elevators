...
public class ElevatorController {
    public boolean isElevator(World world, BlockPos targetPos) {
        return Config.getElevatorBlockMaxHeightMap()
                .containsKey(world.getBlockState(targetPos).getBlock());
    }
    ...
    private boolean isSpaceFree(World world, BlockPos targetPos) {
        return world.getBlockState(targetPos.up()).isAir()
                && world.getBlockState(targetPos.up().up()).isAir();
    }
    ...
    public boolean tryTeleport(ServerPlayerEntity player, boolean up) {
        ...
        int maxTeleportHeight =
                getMaxTeleportHeight(
                        player.world.getBlockState(player.getPosition().down()).getBlock());
        Optional<BlockPos> target =
                up
                        ? nextUpElevator(
                                player.world, player.getPosition().down(), maxTeleportHeight)
                        : nextDownElevator(
                                player.world, player.getPosition().down(), maxTeleportHeight);
        if (!target.isPresent()) {
            return false;
