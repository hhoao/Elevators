...
public class DefaultDispatcher implements Dispatcher {
    ...
    private final Map<DirectionalMessageInfo<?>, List<MessageHandlerWrapper<? extends Message>>>
    ...
    @Override
    public <T extends Message> String registerMessageHandler(
            org.hhoao.mc.ironelevators.net.common.MessageHandler<T> requestHandler,
            Class<T> messageClass,
            NetworkDirection networkDirection) {
        return registerMessageHandler(
                requestHandler, messageClass, Collections.singletonList(networkDirection));
    }
    ...
    public <T extends Message> String registerMessageHandler(
        ...
        for (NetworkDirection networkDirection : networkDirectionList) {
            DirectionalMessageInfo<T> tDirectionMessage =
                    new DirectionalMessageInfo<>(messageClass, networkDirection);
            if (!registeredMessages.contains(tDirectionMessage)) {
                registeredMessages.add(tDirectionMessage);
                CHANNEL.messageBuilder(messageClass, messageNumber++, networkDirection)
    ...
    public void removeMessageHandler(String id) {
        Collection<List<MessageHandlerWrapper<? extends Message>>> values =
                requestHandlerMap.values();
    ...
    private <T extends Message> void process(
            T request, Supplier<NetworkEvent.Context> contextSupplier) {
        ...
        List<MessageHandlerWrapper<? extends Message>> requestHandlers =
                messageHandlerWrappers.stream()
                        .filter(
        ...
        for (org.hhoao.mc.ironelevators.net.common.MessageHandler<? extends Message>
                requestHandler : requestHandlers) {
            if (requestHandler != null) {
                ((org.hhoao.mc.ironelevators.net.common.MessageHandler<T>) requestHandler)
                        .handle(request, contextSupplier);
            }
        }
    }
    ...
    private static class MessageHandlerWrapper<T extends Message>
        ...
        public MessageHandlerWrapper(
                String id,
                org.hhoao.mc.ironelevators.net.common.MessageHandler<T> innerMessageHandler) {
            this.id = id;
    ...
    private static class DirectionalMessageInfo<T extends Message> {
        ...
        public boolean equals(Object o) {
            ...
            return Objects.equals(messageCls, that.messageCls)
                    && networkDirection == that.networkDirection;
        }
    ...
    private static class InternalMessageHandler<MSG extends Message>
